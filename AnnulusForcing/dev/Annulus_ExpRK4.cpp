/*
    @File       RestartGenerator_ExpRK4.cpp
...
namespace Setup{
    struct JsonConfig{
        ...
        int32_t         Seed_f;                     // Seed for generating Forcing
        double          Lambda_f;                   // Forcing Center Wave Number
        double          Target_f;                   // Forcing L2 Norm Target

        int32_t         Type;                       // Type for generating Initial Condition
        int32_t         Seed_i;                     // Seed for generating Initial Condition
    };
    ...
    void to_json(nlohmann::json& j, const JsonConfig& jc) {
        j = nlohmann::json{
            ...
            {"Seed_f",          jc.Seed_f},
            {"Lambda_f",        jc.Lambda_f},
            {"Target_f",        jc.Target_f},
    ...
    void from_json(const nlohmann::json& j, JsonConfig& jc){
        ...
        j.at("Seed_f").get_to(jc.Seed_f);
        j.at("Lambda_f").get_to(jc.Lambda_f);
        j.at("Target_f").get_to(jc.Target_f);
...
struct Preset{
    ...
    // -- Annulus Forcing
    double      lambda_f;       // center  wave number for Forcing          ( config.json initialized )
    double      lambda_m;       // lowest  wave number for Forcing          ( CONSTANT = Defined Later )
    double      lambda_M;       // highest wave number for Forcing          ( CONSTANT = Defined Later )
    double      target_f;       // Target |f|, L2 Norm                      ( config.json initialized )
    int32_t     seed_f;         // seed for forcing
    ...
    int32_t c_max() { return std::ceil(t_max / dt);  };     // Time to Steps
    int32_t c_out() { return std::ceil(t_out / dt);  };     // Time to Steps
    int32_t c_info(){ return std::ceil(t_info / dt); };     // Time to Steps

};

/*
    [Struct - Data] : Stores data for Calculation ( Temporary Data Allocation )
*/
struct Allocated{
    MatrixCd lin_h;
    MatrixCd adv_h;
...
*/
struct Velocities{
    MatrixRd u;                 // x direction Velocity Field
    MatrixRd v;                 // y direction Velocity Field
    MatrixRd psi;               // Stream Function      Field
    MatrixRd omg;               // Vorticity            Field
};
...
/*
    [Functions - Math] : Calculate L2 Norm of Forcing Field with Vorticity Field on Wavenumber Space.
    @omg_h  [const MatrixCd&] - Vorticity Field on Wavenumber Space.
    @ps     [Preset&]         - Experiment Setups.
*/
double L2Norm_Forcing(MatrixCd& g_h, Preset& ps, double l_m, double l_M){
    // -- Temporary Forcing Field Initialization
    MatrixCd fx_h(ps.Nx, ps.Ny);
    MatrixCd fy_h(ps.Nx, ps.Ny);
    fx_h.setZero();
    fy_h.setZero();
    // -- Calculate Forcing Field on Physical Space
    #pragma omp parallel for collapse(2)
    for(int32_t i = 0; i < ps.Nx; i++){
        for(int32_t j = 0; j < ps.Ny; j++){
            double denom = ps.K(i, j);
            if(i == 0 && j == 0){               // -- Not necessary but readability.
                continue;
            }
            if(l_m <= denom && denom <= l_M){   // -- Only cares inserted wavenumbers.
                fx_h(i, j) = g_h(i, j) * std::complex<double>(0.0, - ps.ky(j)) / denom;
                fy_h(i, j) = g_h(i, j) * std::complex<double>(0.0,   ps.kx(i)) / denom;
            }
        }
    }
    // -- Calculate L2Norm
    return ps.Lx * std::sqrt((fx_h.cwiseAbs2().sum() + fy_h.cwiseAbs2().sum()));
}

/*
    [Functions - Math] : Generate Forcing Field on Wavenumber Space with Target L2Norm and Wavenumbers. 
    @ps     [Preset&] - Experiment Setups.
    @l_m    [double]  - Infimum Wavenumber to apply forcing.
    @l_M    [double]  - Supremum Wavenumber to apply forcing.
    @target [double]  - Target L2Norm to normalize Forcing Field.
*/
MatrixCd GenerateForcing(Preset& ps, double l_m, double l_M, double target){
    
    // -- RNG with Mersenne Twister Engine
    std::mt19937 rnd_src(ps.seed_f);
    // -- RNG with Standard Normal Distribution
    std::normal_distribution<double> dist(0.0, 1.0);

    // -- Forcing Field on Wavenumber Space Initialization
    MatrixCd g_h(ps.Nx, ps.Ny);
    g_h.setZero();

    // DONT PALALLELIZE HERE!! NOT THREAD SAFE!!
    // -- Generate Random Forcing Coefficients
    for(int32_t i = 0; i < ps.Nx; i++){
        for(int32_t j = 0; j < ps.Ny / 2 + 1; j++){
            double wn = ps.K(i, j);
            if(l_m <= wn && wn <= l_M){
                double re = dist(rnd_src);
                double im = dist(rnd_src);
                g_h(i, j) = std::complex<double>(re, im);
            } 
        }
    }

    // -- Copy to restore whole matrix
    // -- >> Top Right (First Row)
    g_h.row(0).segment(ps.Ny / 2 + 1, ps.Ny / 2 - 1) = g_h.row(0).segment(1, ps.Ny / 2 - 1).conjugate().reverse();
    // -- >> Lower Right
    g_h.bottomRows(ps.Nx - 1).rightCols(ps.Ny / 2 - 1) = g_h.bottomRows(ps.Nx - 1).middleCols(1, ps.Ny / 2 - 1).conjugate().colwise().reverse().rowwise().reverse();

    // -- Scale Matrix to match Target L2Norm
    double scaler = L2Norm_Forcing(g_h, ps, l_m, l_M);
    MatrixCd scaled_g_h = g_h / scaler * target;

    std::cout << "-- Target = " << ps.target_f << std::endl;
    std::cout << "-- |f|    = " << L2Norm_Forcing(scaled_g_h, ps, l_m, l_M) << std::endl;
...
double CFL(MatrixCd& omg_h, Preset& ps, IFFT2D_REAL& inv){
    ...
    return ps.Nx * ps.dt / (2.0 * ps.Lx) * (u.cwiseAbs() + v.cwiseAbs()).maxCoeff();
}
...
int32_t main(int32_t argc, char** argv){
    ...
    */
    ...
    // -- Forcing params
    ps.seed_f       = config.Seed_f;
    ps.target_f     = config.Target_f;

    ps.lambda_f     = config.Lambda_f;
    double diff     = 4.0 * std::sqrt(ps.lambda_f) - 2.0;
    ps.lambda_m     = ps.lambda_f - (diff / 2.0);
    ps.lambda_M     = ps.lambda_f + (diff / 2.0);
    ...
    */
    ...
    // -- g_h Generation
    MatrixCd g_h = GenerateForcing(ps, ps.lambda_m, ps.lambda_M, ps.target_f);
